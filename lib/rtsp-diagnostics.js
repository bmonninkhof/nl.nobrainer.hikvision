...
function observeInterleavedRtp(socket, { timeout = 1500, channel = 0 } = {}) {
...
async function probeRtspPlayback({
  ...
  try {
    ...
    if (play.statusCode === 200) result.media = await observer(socket, { timeout: Math.min(2000, timeout), channel: 0 });
  } catch (error) {
