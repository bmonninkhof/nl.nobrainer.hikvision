...
const { LOCAL_DISPLAY_PROBES, summarizeLocalDisplayProbe } = require('./local-display-diagnostics');
...
const LOCAL_DISPLAY_PROBE_TIMEOUT = 3000;
const LOCAL_DISPLAY_DIAGNOSTIC_TIMEOUT = 4000;
...
class HikvisionClient extends EventEmitter {
  ...
  async getLocalDisplayDiagnostics({
    timeout = LOCAL_DISPLAY_PROBE_TIMEOUT,
    overallTimeout = LOCAL_DISPLAY_DIAGNOSTIC_TIMEOUT,
  } = {}) {
    const diagnostics = {
    ...
    const probeTasks = LOCAL_DISPLAY_PROBES.map(async probe => {
      let result;
      try {
        const response = await this.request(probe.path, {
          method: 'GET',
          timeout,
        ...
        if (contentType.includes('json') || body.trimStart().startsWith('{')) {
          ...
          format = 'xml';
          const responseError = getIsapiResponseError(parsed);
          if (responseError) {
            const error = new Error(responseError.statusString);
            error.code = 'EISAPIRESPONSE';
            error.isapiStatusCode = responseError.statusCode;
            throw error;
          }
        ...
        result = {
          status: 'available',
          httpStatus: Number(response.statusCode) || 200,
          result: summarizeLocalDisplayProbe(probe.id, parsed, format),
        };
      } catch (error) {
        result = {
          status: 'unavailable',
          ...
          isapiStatusCode: Number(error?.isapiStatusCode) || null,
        };
      }
      completed.set(probe.id, result);
    ...
    const results = await Promise.race([
      ...
      new Promise(resolve => {
        timer = setTimeout(() => resolve(null), Math.max(1, Number(overallTimeout) || LOCAL_DISPLAY_DIAGNOSTIC_TIMEOUT));
      }),
    ]);
    if (timer) clearTimeout(timer);

    diagnostics.timedOut = results === null;
    diagnostics.probes = Object.fromEntries(LOCAL_DISPLAY_PROBES.map(probe => [
      probe.id,
      completed.get(probe.id) || {
        status: 'unavailable',
        errorCode: 'ETIMEDOUT',
        httpStatus: null,
        isapiStatusCode: null,
      },
    ]));
    return diagnostics;
  }
  ...
  async searchRecordings({ channel = 1, startTime, endTime, offset = 0, maxResults = 20 } = {}) {
    const channelId = Number(channel);
    const start = new Date(startTime);
    const end = new Date(endTime);
    if (!Number.isInteger(channelId) || channelId < 1 || channelId > 999
      || !Number.isFinite(start.getTime()) || !Number.isFinite(end.getTime())
      || start >= end) throw new Error('Invalid recording search');
    const limit = Math.max(1, Math.min(40, Number(maxResults) || 20));
    const position = Math.max(0, Number(offset) || 0);
    const trackId = Number(`${channelId}01`);
    const body = '<?xml version="1.0" encoding="UTF-8"?>'
      ...
      + `<timeSpanList><timeSpan><startTime>${start.toISOString()}</startTime><endTime>${end.toISOString()}</endTime></timeSpan></timeSpanList>`
      + `<maxResults>${limit}</maxResults><searchResultPostion>${position}</searchResultPostion>`
      + '<metadataList><metadataDescriptor>//recordType.meta.std-cgi.com</metadataDescriptor></metadataList>'
      + '</CMSearchDescription>';
    const response = await this.request('/ISAPI/ContentMgmt/search', {
      method: 'POST',
      body,
    ...
    const responseError = getIsapiResponseError(xml);
    if (responseError) {
      const error = new Error(`Hikvision recording search failed: ${responseError.statusString}`);
      error.code = 'EISAPIRECORDINGSEARCH';
      throw error;
    }
    const matches = asList(findValue(xml, 'searchMatchItem'));
    return {
      status: String(findValue(xml, 'responseStatusStrg') || findValue(xml, 'responseStatus') || 'OK'),
      totalMatches: Number(findValue(xml, 'numOfMatches')) || matches.length,
      recordings: matches.map(item => ({
        channel: channelId,
        trackId: Number(findValue(item, 'trackID')) || trackId,
        startTime: String(findValue(item, 'startTime') || ''),
        endTime: String(findValue(item, 'endTime') || ''),
        codec: String(findValue(item, 'codecType') || ''),
        recordingType: String(findValue(item, 'metadataDescriptor') || ''),
        playbackUri: String(findValue(item, 'playbackURI') || ''),
      })).filter(item => item.startTime && item.endTime && item.playbackUri),
    };
  }
  ...
  async getStreamingDiagnostics(channelId, streamIndex = 1) {
    const streamId = Number(`${channelId}0${streamIndex}`);
    const xml = await this.getXml(`/ISAPI/Streaming/channels/${streamId}`);
    const number = (...keys) => {
      for (const key of keys) {
        const value = Number(findValue(xml, key));
    ...
    const text = (...keys) => {
      for (const key of keys) {
        const value = findValue(xml, key);
        if (value !== undefined && value !== null && String(value).trim()) return String(value).trim();
      }
      return null;
    };
    const rawFrameRate = number('maxFrameRate', 'frameRate');
    return {
      readOnly: true,
      streamId,
      codec: text('videoCodecType'),
      profile: text('H264Profile', 'h264Profile'),
      width: number('videoResolutionWidth'),
      height: number('videoResolutionHeight'),
      frameRate: rawFrameRate !== null && rawFrameRate > 100
        ? Number((rawFrameRate / 100).toFixed(2))
        : rawFrameRate,
      bitrateControl: text('videoQualityControlType', 'bitrateType'),
      constantBitrateKbps: number('constantBitRate'),
      maximumBitrateKbps: number('vbrUpperCap', 'maxBitRate', 'videoBitRate'),
      quality: number('fixedQuality', 'videoQuality'),
      keyframeInterval: number('GovLength', 'govLength', 'keyFrameInterval'),
      smartCodec: text('SmartCodec', 'smartCodec', 'smartCodecEnabled'),
      svc: text('SVC', 'svc', 'svcEnabled'),
    };
  }
  ...
  async _handleAlertXml(xml) {
    ...
    this.emit('event-diagnostic', {
      eventType: codeValue,
      mappedType: mappedCode || null,
      eventState: state,
      channel,
      activePostCount: hasCount && Number.isFinite(count) ? count : null,
    });
